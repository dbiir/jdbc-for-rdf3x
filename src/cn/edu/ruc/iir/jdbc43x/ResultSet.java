...
/** 
 * This <code>ResultSet</code> implements the java.sql.ResultSet interface.
 * It reads the result one by one from an opened input stream of rdf3xembedded.
 * So it does not load all the data into memory and does not cause a OutOfMemory 
 * error. Most important, this ResultSet is 3 or 4 times faster than the de.mpii.rdf3x.ResultSet.
 * But at the same time, it could only read sequentially with the next() function.
 * 
...
final class ResultSet implements java.sql.ResultSet
{
	...
	// Constructor
	ResultSet(String[] header, Connection connection, FunctionCallback callback)
	{
		this.header = header;
		this.currow = null;
	...
	{
		return new ResultSetMetaData(header);
	}
	...
	{
		String s = null;
		if ((columnIndex < 1) || (columnIndex > currow.length))
		{
			s = currow[0];
	...
	public boolean next() throws SQLException
	{
		...
		while (true)
		{
			tmprow = connection.readResultLine();
			if (tmprow == null)
			{
				currow = null;
				return false;
			}
			if ((tmprow.length >= 4) && ("callback".equals(tmprow[0])))
			{
				java.util.List<String[]> values;
				int columns;
				try
				{
					columns = Integer.parseInt(tmprow[3]);
				...
				{
					String[] args = new String[tmprow.length - 4];
					System.arraycopy(tmprow, 4, args, 0, args.length);
					values = callback.eval(tmprow[2], args);
				}
				connection.writeResultLine(new String[]
				{ "ok", tmprow[1] });
				if (values != null)
				{
					for (String[] l : values)
